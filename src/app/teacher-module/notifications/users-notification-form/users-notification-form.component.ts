...
export class UsersNotificationFormComponent implements OnInit {
  ...
  onConfirmNotification() {
    if (this.selected == Library.STUDENT) {
      ...
      _.map(this.selectedOptions, (selectedGroup: Group) => this.executeQuerys(selectedGroup));
    }
  }
  ...
  openDialog(): void {
    ...
    const dialogMessage = `Vous allez notifier ${selectedOptionNumber} ${seletedChoice}. ` +
                          `S'il vous plaît, cliquez sur confirmer pour continuer?`;
